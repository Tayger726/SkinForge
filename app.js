...
function bindMarketComparison(s){
 ...
 const update=()=>{
  ...
  $$('.market-compare-row[data-market]',box).forEach(row=>{
   ...
   if(row.dataset.kind==='cash'&&row.dataset.stale!=='true'&&price>0&&(!best||net>best.net))best={row,net,name:$('.market-title strong',row).textContent};
  });
  if(best){best.row.classList.add('market-best');$('#marketBest').innerHTML=`<small>Наибольшая расчётная сумма • не гарантия продажи</small><strong>${best.name} · ${money(best.net)}</strong>`}else $('#marketBest').textContent='Цены временно недоступны';
 };
