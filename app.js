...
function analyticsAttribution(){try{const key='aerox_analytics_attribution',params=new URLSearchParams(location.search),source=params.get('utm_source'),campaign=params.get('utm_campaign');let x=JSON.parse(localStorage.getItem(key)||'null');if(source){x={source,campaign:campaign||'',created:Date.now()};localStorage.setItem(key,JSON.stringify(x))}else if(!x&&document.referrer){const ref=new URL(document.referrer);if(ref.origin!==location.origin){x={source:ref.hostname.replace(/^www\./,''),campaign:'',created:Date.now()};localStorage.setItem(key,JSON.stringify(x))}}if(x&&Date.now()-Number(x.created||0)<30*86400000)return{source:String(x.source||'').slice(0,80),campaign:String(x.campaign||'').slice(0,100)};return{}}catch(e){return{}}}
function trackEvent(eventName,details={}){if(localStorage.getItem('aerox_analytics_optout')==='1')return;const visitor=analyticsVisitor();if(!visitor)return;const payload={visitor_id:visitor,event_name:eventName,page:location.pathname||'/',item_name:String(details.item_name||'').slice(0,220)||undefined,market:String(details.market||'').slice(0,40)||undefined,action:String(details.action||'').slice(0,16)||undefined,...analyticsAttribution()};fetch('/api/events',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),keepalive:true}).catch(()=>{})}
...
function readSavedList(key){try{const v=JSON.parse(localStorage.getItem(key)||'[]');return Array.isArray(v)?v.filter(x=>x&&typeof x==='object'&&typeof x.id==='string'):[]}catch(e){return[]}}
function portfolio(){return readSavedList('skinforge_portfolio')}
...
function dealScore(s,feePct=8){
  ...
  const verdict=discount>0?'Ниже ориентира':'Без скидки к ориентиру';
...
async function loadLiveData(){
  try{
    ...
    const skinportStale=itemsRes.headers.get('X-Aerox-Data-Stale')==='1',skinportDataTime=itemsRes.headers.get('X-Aerox-Data-Time')||null;

    const live=(Array.isArray(items)?items:[]).map(i=>{
      try{return skinportToSkin(i)}catch(e){console.warn('AEROX: bad Skinport item skipped',i?.market_hash_name,e);return null}
    }).filter(Boolean);
    live.forEach(s=>{s.marketStale=skinportStale;s.marketUpdatedAt=skinportDataTime});
    window.LIVE_SKINS=live;
    window.CLEAN_SKINS=live.filter(isUsableLiveSkin);
    SF_DIAG.items=live.length;SF_DIAG.clean=window.CLEAN_SKINS.length;SF_DIAG.lastUpdate=skinportDataTime||new Date().toISOString();SF_DIAG.fallback=skinportStale;
...
function renderCatalog(){
 ...
 ['searchInput','category','weaponFilter','conditionFilter','stattrakFilter','minPrice','maxPrice','sort','onlyFav'].forEach(id=>{const el=$('#'+id);if(!el)return;el.addEventListener(id==='onlyFav'?'change':'input',()=>{page=1;run()})});run();document.addEventListener('skinforge-live-ready',()=>{page=1;run()});document.addEventListener('skinforge-history-ready',run);document.addEventListener('skinforge-watchlist-change',run);
}
function chart(canvas,s,days=7){
 // Skinport supplies rolling aggregates, not timestamped price observations.
 // Never turn averages or the current listing into a fabricated time series.
 canvas.hidden=true;
 const host=canvas.parentElement;
 let note=host.querySelector('.history-data-note');
 if(!note){note=document.createElement('div');note.className='history-data-note';note.style.cssText='padding:24px;line-height:1.7';host.appendChild(note)}
 const key=days===1?'last_24_hours':days===7?'last_7_days':'last_30_days';
 const h=s.history?.[key],avg=Number(h?.avg);
 note.replaceChildren();
 const title=document.createElement('strong');title.textContent='Недостаточно данных для графика';note.appendChild(title);
 const detail=document.createElement('p');detail.textContent='Источник передаёт сводку продаж за периоды, а не последовательную историю цены. Поэтому линия изменения цены не отображается.';note.appendChild(detail);
 if(Number.isFinite(avg)&&avg>0){const value=document.createElement('p');value.textContent='Средняя цена продаж за '+(days===1?'24 часа':days+' дней')+': '+money(avg);note.appendChild(value)}
}
...
function marketDataLabel(data){
 if(!data)return 'Нет данных источника';
 const raw=data.updated_at||data.fetched_at,stamp=raw?Date.parse(raw):NaN;
 const age=Number.isFinite(stamp)?Math.max(0,Date.now()-stamp):null,relative=age==null?'время неизвестно':age<60000?'только что':age<3600000?`${Math.floor(age/60000)} мин назад`:age<86400000?`${Math.floor(age/3600000)} ч назад`:`${Math.floor(age/86400000)} дн назад`;
 return (data.stale?'Сохранённая копия • ':'' )+(data.updated_at?'Обновлено ':'Получено ')+relative;
}
function exactMarketUrl(market,s){const hash=String(s?.marketHash||s?.hash||s?.name||'').trim();if(market==='skinport')return s?.itemPage||s?.marketPage||`https://skinport.com/market?search=${encodeURIComponent(hash)}`;if(market==='csfloat')return`https://csfloat.com/search?market_hash_name=${encodeURIComponent(hash)}`;if(market==='dmarket')return`https://dmarket.com/ingame-items/item-list/csgo-skins?title=${encodeURIComponent(hash)}`;if(market==='steam')return`https://steamcommunity.com/market/listings/730/${encodeURIComponent(hash)}`;return'#'}
function marketComparisonMarkup(s){
 ...
 const rows=[
  {id:'skinport',name:'Skinport',kind:'cash',status:'Цена объявления, не ордер покупателя',price:Number(s.price)||0,fee:8,readonly:true,url:exactMarketUrl('skinport',s)},
  {id:'steam',name:'Steam Market',kind:'wallet',status:steam?(prices.steam?.price_type||'Средняя продаж')+' • '+marketDataLabel(prices.steam):'Цена временно недоступна',price:steam,fee:15,url:exactMarketUrl('steam',s)},
  {id:'csfloat',name:'CSFloat',kind:'cash',status:csfloat?'Ордер из сводки • '+marketDataLabel(prices.csfloat):'Цена временно недоступна',price:csfloat,fee:2,url:exactMarketUrl('csfloat',s)},
  {id:'dmarket',name:'DMarket',kind:'cash',status:'Цена временно недоступна',price:0,fee:2,disabled:true,url:exactMarketUrl('dmarket',s)}
 ];
 return `<section class="market-compare" id="marketCompare"><div class="market-compare-head"><div><div class="eyebrow">AEROX SELL COMPARE</div><h3>Расчёт продажи</h3><p>Подтверждённым считается только существующий ордер покупателя. Остальные строки — ориентиры.</p></div><div class="market-best-result" id="marketBest">Ищем ордер покупателя…</div></div><div class="market-compare-table"><div class="market-compare-row market-compare-labels"><span>Площадка</span><span>Цена</span><span>Комиссия</span><span>Расчёт после комиссии</span><span></span></div>${rows.map(r=>`<div class="market-compare-row${r.disabled?' market-unavailable':''}" data-market="${r.id}" data-kind="${r.kind}" data-stale="${prices[r.id]?.stale===true}"><span class="market-title"><strong>${r.name}</strong><small>${r.status}${r.kind==='wallet'?' • Steam Wallet':' • реальные деньги'}</small></span><label><small>Цена, $</small><input class="market-price" type="number" min="0" step="0.01" value="${r.price||''}" placeholder="Нет данных" readonly></label><label><small>Комиссия, %</small><input class="market-fee" type="number" min="0" max="99.99" step="0.5" value="${r.fee}" ${r.disabled?'disabled':''}></label><strong class="market-net">${r.disabled?'Недоступно':'—'}</strong><a class="secondary market-go" href="${r.url}" target="_blank" rel="noopener noreferrer">Открыть</a></div>`).join('')}</div><div class="notice">Skinport — цена объявления, CSFloat — ордер покупателя, Steam — средняя цена продаж в Steam Wallet. Только актуальный ордер CSFloat участвует в подтверждении продажи. Цена и доступность сделки требуют проверки на площадке.</div></section>`;
}
function bindMarketComparison(s){
 ...
 const update=()=>{
  ...
  $$('.market-compare-row[data-market]',box).forEach(row=>{
   const price=Number($('.market-price',row)?.value),fee=Number($('.market-fee',row)?.value);row.classList.remove('market-best');if(!Number.isFinite(price)||price<=0){$('.market-net',row).textContent='Недоступно';return;}if(!Number.isFinite(fee)||fee<0||fee>=100){$('.market-net',row).textContent='Комиссия: от 0 до <100%';return;}const net=price*(1-fee/100);
   ...
   if(row.dataset.market==='csfloat'&&row.dataset.stale!=='true'&&price>0&&(!best||net>best.net))best={row,net,name:$('.market-title strong',row).textContent};
  });
  if(best){best.row.classList.add('market-best');$('#marketBest').innerHTML=`<small>Текущий ордер покупателя • после комиссии</small><strong>${best.name} · ${money(best.net)}</strong>`}else $('#marketBest').textContent='Нет актуального ордера покупателя';
 };
...
function renderSkin(){
 ...
 const draw=()=>{
   ...
   <div class="quick-actions"><button class="ghost fav" data-id="${htmlEsc(s.id)}">♥ В избранное</button><button class="primary" id="alertAdd">🔔 Следить за ценой</button></div>
   ...
   <h3>Сравнение со Steam</h3><div class="panel"><div class="market-row"><strong>Steam — средняя цена продаж</strong><span>${s.steam?money(st):(s._marketAttempted?'Цена недоступна':'загрузка...')}</span><a class="primary" href="${steamUrl}" target="_blank">Steam</a></div><div class="market-row"><strong>Разница Steam − Skinport</strong><span class="${diff!=null?(diff>=0?'up':'down'):''}">${diff==null?'—':`${diff>=0?'+':''}${money(diff)}`}</span></div></div><p class="muted">Steam Wallet и cash-market Skinport — разные типы стоимости.</p>${marketComparisonMarkup(s)}</div>`;
...
async function renderArbitrage(){
  ...
  const drawRows=(items)=>{
    ...
    $('#scanSteam')?.addEventListener('click',async()=>{
      ...
      const names=items.filter(s=>s.live).map(s=>s.marketHash||s.hash).slice(0,8);
      try{
        const r=await fetch('/api/steam-batch?names='+encodeURIComponent(JSON.stringify(names)));
        ...
        items.forEach(s=>{
          ...
          const d=byName.get(s.marketHash||s.hash);
...
function renderPortfolio(){
  ...
  const draw=()=>{
    ...
    root.innerHTML=`
      ...
      <div class="notice">Без входа данные хранятся в браузере. После входа доступна синхронизация с аккаунтом.</div>`;
...
function verifiedDealSnapshot(s,options={}){
 ...
 const sells={csfloat:valid(prices.csfloat?.sell,prices.csfloat)};
 const buyMarket=options.buyMarket&&buys[options.buyMarket]?options.buyMarket:Object.entries(buys).filter(x=>x[1]).sort((a,b)=>a[1]-b[1])[0]?.[0];
 const sellMarket=options.sellMarket&&sells[options.sellMarket]?options.sellMarket:Object.entries(sells).filter(x=>x[1]).sort((a,b)=>b[1]-a[1])[0]?.[0];
 const buy=buys[buyMarket],sell=sells[sellMarket],buyFee=Number(options.buyFee??0),sellFee=Number(options.sellFee??2),reservePct=Number(options.reservePct??5);
 if(!buy||!sell)return{ok:false,reason:!buy?'Нет актуального cash-предложения покупки':'Нет актуального ордера покупателя',buys,sells,buyMarket,sellMarket};
 if(![buyFee,sellFee,reservePct].every(Number.isFinite)||buyFee<0||buyFee>=100||sellFee<0||sellFee>=100||reservePct<0||reservePct>=100)return{ok:false,reason:'Проверь комиссии и резерв риска',buys,sells,buyMarket,sellMarket};
 const cost=buy*(1+buyFee/100),net=sell*(1-sellFee/100),reserve=cost*reservePct/100,profit=net-cost-reserve,roi=cost?profit/cost*100:0;
 return{ok:true,buys,sells,buyMarket,sellMarket,buy,sell,buyFee,sellFee,reservePct,cost,net,reserve,profit,roi,positive:profit>0};
}
function saveProfitAlert(s,minProfit=1,minRoi=0,reservePct=5){
...
function checkPriceAlerts(){const source=window.LIVE_SKINS||[];let a=alerts(),changed=false;a.forEach(x=>{if(x.kind==='profit')return;const s=source.find(k=>(k.marketHash||k.hash)===x.hash||k.id===x.skinId);if(!s||s.price==null)return;const hit=x.direction==='below'?s.price<=x.target:s.price>=x.target;if(hit&&!x.triggered){x.triggered=true;x.triggeredAt=Date.now();x.lastPrice=s.price;changed=true;setTimeout(()=>toast(`🔔 ${s.name}: ${money(s.price)} — цель ${money(x.target)}`),200)}});if(changed)setAlerts(a)}
async function refreshProfitAlerts(){
 const source=window.LIVE_SKINS||[],a=alerts(),pending=a.filter(x=>x.kind==='profit'&&!x.triggered&&x.hash).slice(0,20);if(!pending.length)return;
 const byHash=new Map();await Promise.all([...new Set(pending.map(x=>x.hash))].map(async hash=>{try{const r=await fetch('/api/market-prices?name='+encodeURIComponent(hash));if(r.ok)byHash.set(hash,(await r.json()).markets||{})}catch(e){}}));
 let changed=false;for(const x of pending){const s=source.find(k=>(k.marketHash||k.hash)===x.hash||k.id===x.skinId);if(!s||!byHash.has(x.hash))continue;s.marketPrices=byHash.get(x.hash);const d=verifiedDealSnapshot(s,{reservePct:x.reservePct});x.lastCheckedAt=Date.now();x.lastProfit=d.ok?d.profit:null;x.lastRoi=d.ok?d.roi:null;x.lastReason=d.ok?(d.positive?'Есть положительная маржа':'После расходов получается убыток'):d.reason;const hit=d.ok&&d.profit>=Number(x.minProfit||0)&&d.roi>=Number(x.minRoi||0);if(hit){x.triggered=true;x.triggeredAt=Date.now();x.buyPrice=d.buy;x.sellOrder=d.sell;setTimeout(()=>toast(`🔔 ${s.name}: расчётная прибыль ${money(d.profit)} (${d.roi.toFixed(1)}%)`),200)}changed=true}if(changed){setAlerts(a);document.dispatchEvent(new Event('skinforge-alerts-change'))}
}
function renderFavorites(){const root=$('#favoritesRoot');if(!root)return;const draw=()=>{const list=watchlist();if(!list.length){root.innerHTML='<div class="empty watch-empty"><strong>Список наблюдения пуст</strong><span>Добавь ♥ у нужного скина в каталоге — AEROX запомнит цену и покажет изменение.</span><a class="primary" href="catalog.html">Открыть каталог</a></div>';return}const live=Array.isArray(window.LIVE_SKINS)?window.LIVE_SKINS:[];if(!live.length&&window.LIVE_READY!==false){root.innerHTML=`<div class="watch-loading">${skeletonCards(Math.min(6,list.length))}</div>`;return}let upgraded=false;list.forEach(entry=>{const s=live.find(x=>x.id===entry.id||(entry.hash&&String(x.marketHash||x.hash||'')===entry.hash));if(s&&!(Number(entry.addedPrice)>0)){Object.assign(entry,{hash:s.marketHash||s.hash||entry.hash||'',name:s.name||entry.name||'',condition:s.condition||entry.condition||'',image:s.img||entry.image||'',addedPrice:Number(s.price)||null,addedAt:entry.addedAt||Date.now()});upgraded=true}});if(upgraded)setWatchlist(list);const rows=list.map(entry=>{const s=live.find(x=>x.id===entry.id||(entry.hash&&String(x.marketHash||x.hash||'')===entry.hash));const current=Number(s?.price),added=Number(entry.addedPrice),target=Number(entry.targetPrice),change=Number.isFinite(current)&&added>0?current-added:null,pct=change!=null?change/added*100:null,reached=target>0&&Number.isFinite(current)&&current<=target;const name=s?.name||entry.name||'Сохранённый скин',condition=s?.condition||entry.condition||'',img=s?.img||entry.image||'',href=`skin.html?id=${encodeURIComponent(entry.id)}${entry.hash?'&hash='+encodeURIComponent(entry.hash):''}`;return `<article class="watch-row" data-watch-id="${htmlEsc(entry.id)}"><div class="watch-skin">${img?`<img src="${htmlEsc(img)}" alt="${htmlEsc(name)}" loading="lazy" onerror="this.style.opacity=.15">`:''}<div><a href="${href}"><strong>${htmlEsc(name)}</strong></a><small>${htmlEsc(condition||'—')}</small></div></div><div class="watch-value"><small>При добавлении</small><strong>${added>0?money(added):'—'}</strong></div><div class="watch-value"><small>Сейчас</small><strong>${Number.isFinite(current)?money(current):'—'}</strong></div><div class="watch-value"><small>Изменение</small><strong class="${change==null?'':change>=0?'up':'down'}">${change==null?'—':`${change>=0?'+':''}${money(change)} · ${pct>=0?'+':''}${pct.toFixed(1)}%`}</strong></div><label class="watch-target"><small>Купить при цене ≤ $</small><input type="number" min="0" step="0.01" value="${target>0?target:''}" placeholder="Задать цель" data-watch-target="${htmlEsc(entry.id)}"><em class="${reached?'reached':''}">${reached?'Цель достигнута':'Наблюдение'}</em></label><div class="watch-actions"><a class="secondary" href="${href}">Открыть</a><button class="danger watch-remove" type="button" data-watch-remove="${htmlEsc(entry.id)}">Удалить</button></div></article>`}).join('');const priced=list.map(e=>{const s=live.find(x=>x.id===e.id||(e.hash&&String(x.marketHash||x.hash||'')===e.hash));return{added:Number(e.addedPrice),current:Number(s?.price)}}).filter(x=>x.added>0&&Number.isFinite(x.current));const addedTotal=priced.reduce((a,x)=>a+x.added,0),currentTotal=priced.reduce((a,x)=>a+x.current,0),delta=currentTotal-addedTotal;root.innerHTML=`<section class="watch-summary"><div><span>В наблюдении</span><strong>${list.length}</strong></div><div><span>Цена при добавлении</span><strong>${priced.length?money(addedTotal):'—'}</strong></div><div><span>Текущая стоимость</span><strong>${priced.length?money(currentTotal):'—'}</strong></div><div><span>Общее изменение</span><strong class="${delta>=0?'up':'down'}">${priced.length?`${delta>=0?'+':''}${money(delta)}`:'—'}</strong></div></section><div class="watch-table">${rows}</div><p class="watch-note">Расчёт показывает изменение рыночной цены одного экземпляра каждого скина. Это аналитика, а не гарантия прибыли.</p>`;root.querySelectorAll('[data-watch-target]').forEach(input=>input.addEventListener('input',()=>{const id=input.dataset.watchTarget,next=watchlist(),entry=next.find(x=>x.id===id);if(!entry)return;const value=Number(input.value);entry.targetPrice=Number.isFinite(value)&&value>0?value:null;entry.updatedAt=Date.now();setWatchlist(next);const status=input.parentElement.querySelector('em'),skin=live.find(x=>x.id===id),hit=entry.targetPrice>0&&Number(skin?.price)>0&&Number(skin.price)<=entry.targetPrice;if(status){status.textContent=hit?'Цель достигнута':'Наблюдение';status.classList.toggle('reached',hit)}}));root.querySelectorAll('[data-watch-remove]').forEach(button=>button.addEventListener('click',()=>toggleFav(button.dataset.watchRemove)));renderFavStates()};draw();document.addEventListener('skinforge-live-ready',draw);document.addEventListener('skinforge-images-ready',draw);document.addEventListener('skinforge-watchlist-change',draw);document.addEventListener('skinforge-cloud-ready',draw)}
function renderAlerts(){const root=$('#alertsRoot');if(!root)return;let refreshing=false;const draw=()=>{const a=alerts();root.innerHTML=a.length?`<div class="arb-table"><div class="arb-head"><span>Скин</span><span>Условие</span><span>Цель</span><span>Статус</span><span></span></div>${a.map(x=>{const profit=x.kind==='profit';const checked=x.lastCheckedAt?new Date(x.lastCheckedAt).toLocaleString('ru-RU'):'ещё не проверено';return`<div class="arb-row"><span><strong>${htmlEsc(x.name)}</strong><small>${htmlEsc(x.condition||'')}</small></span><span>${profit?'Прибыль после комиссий и резерва':x.direction==='below'?'Цена ≤':'Цена ≥'}<small>${profit?`Проверено: ${checked}`:''}</small></span><span>${profit?`${money(x.minProfit||0)} · ROI ${Number(x.minRoi||0).toFixed(1)}%`:money(x.target)}</span><span class="${x.triggered?'up':profit&&x.lastProfit<0?'down':''}">${x.triggered?'Сработало':profit&&x.lastProfit!=null?`${money(x.lastProfit)} · ${Number(x.lastRoi||0).toFixed(1)}%`:'Активно'}<small>${profit?htmlEsc(x.lastReason||'Проверяем предложения и ордер покупателя'):''}</small></span><span><button class="danger alert-remove" data-id="${htmlEsc(x.id)}">Удалить</button></span></div>`}).join('')}</div><p class="notice">Умные уведомления проверяются при открытии этой страницы. Они используют только актуальные cash-предложения и ордер покупателя CSFloat; результат через 7 дней не гарантируется.</p>`:'<div class="empty">Уведомлений пока нет. Создай его на странице скина.</div>';$$('.alert-remove').forEach(b=>b.onclick=()=>removeAlert(b.dataset.id))};const refresh=async()=>{if(refreshing)return;refreshing=true;await refreshProfitAlerts();refreshing=false};draw();refresh();document.addEventListener('skinforge-alerts-change',draw);document.addEventListener('skinforge-live-ready',()=>{draw();refresh()})}
...
function renderCalculator(){const root=$('#calcRoot');if(!root)return;root.innerHTML=`<div class="calc-grid"><label>Цена покупки $<input id="cBuy" type="number" value="100" step="0.01"></label><label>Цена продажи $<input id="cSell" type="number" value="120" step="0.01"></label><label>Количество<input id="cQty" type="number" value="1" min="1"></label><label>Комиссия покупки %<input id="cBuyFee" type="number" value="0" step="0.1"></label><label>Комиссия продажи %<input id="cSellFee" type="number" value="12" step="0.1"></label><label>Доп. расходы $<input id="cExtra" type="number" value="0" step="0.01"></label></div><div id="calcResult" class="portfolio-summary"></div>`;const calc=()=>{const buy=+$('#cBuy').value||0,sell=+$('#cSell').value||0,qty=+$('#cQty').value||0,bf=+$('#cBuyFee').value||0,sf=+$('#cSellFee').value||0,ex=+$('#cExtra').value||0;const cost=buy*qty*(1+bf/100)+ex,net=sell*qty*(1-sf/100),profit=net-cost,roi=cost?profit/cost*100:0,be=qty&&sf<100?(cost/qty)/(1-sf/100):null;if(![buy,sell,qty,bf,sf,ex].every(Number.isFinite)||buy<0||sell<0||qty<=0||bf<0||bf>=100||sf<0||sf>=100||ex<0){$('#calcResult').innerHTML='<div class="notice" role="alert">Укажи неотрицательные цены, количество больше нуля и комиссии от 0 до менее 100%.</div>';return;}$('#calcResult').innerHTML=`<div class="stat-box"><span>Затраты</span><strong>${money(cost)}</strong></div><div class="stat-box"><span>Получишь после комиссии</span><strong>${money(net)}</strong></div><div class="stat-box"><span>Чистая прибыль</span><strong class="${profit>=0?'up':'down'}">${profit>=0?'+':''}${money(profit)}</strong></div><div class="stat-box"><span>ROI</span><strong class="${roi>=0?'up':'down'}">${roi.toFixed(1)}%</strong></div><div class="stat-box"><span>Безубыточная продажа</span><strong>${money(be)}</strong></div>`};$$('#calcRoot input').forEach(i=>i.addEventListener('input',calc));calc()}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.addEventListener('click',e=>{const link=e.target.closest?.('a[target="_blank"]');if(!link)return;const row=link.closest('[data-buy-market],[data-market]'),market=row?.dataset.buyMarket||row?.dataset.market;if(market)trackEvent('market_outbound',{market,action:row.dataset.buyMarket?'buy':'sell',item_name:document.querySelector('#skinDetail h1')?.textContent||''})});
