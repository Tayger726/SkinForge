...
function renderSkin(){
 ...
 const draw=()=>{
   ...
   <h3>Сравнение со Steam</h3><div class="panel"><div class="market-row"><strong>Steam — средняя цена продаж</strong><span>${s.steam?money(st):'загрузка...'}</span><a class="primary" href="${steamUrl}" target="_blank">Steam</a></div><div class="market-row"><strong>Разница Steam − Skinport</strong><span class="${diff!=null?(diff>=0?'up':'down'):''}">${diff==null?'—':`${diff>=0?'+':''}${money(diff)}`}</span></div></div><p class="muted">Steam Wallet и cash-market Skinport — разные типы стоимости.</p>${marketComparisonMarkup(s)}</div>`;
